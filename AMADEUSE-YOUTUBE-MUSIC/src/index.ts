...
let icon = 'logo_no_background.png';
...
async function createMainWindow() {
  ...
  win.setTitle(APP_BRAND.productName);
  await initHook(win);
  initTheme(win);
...
app.whenReady().then(async () => {
  ...
  app.setAboutPanelOptions({
    applicationName: APP_BRAND.productName,
    applicationVersion: app.getVersion(),
    authors: [APP_BRAND.author],
    website: APP_BRAND.homepage,
    credits: `${APP_BRAND.productName} integrates the official VRChat OSC plugin.`,
    license: 'MIT',
  });
      ...
      try {
        ...
        shell.writeShortcutLink(
          ...
          error === 'needUpdate' ? 'update' : 'create',
          {
            ...
            description: APP_BRAND.shortcutDescription,
  ...
  if (!is.dev() && config.get('options.autoUpdates')) {
    ...
    autoUpdater.on('update-available', () => {
      const downloadLink = APP_BRAND.downloadLatest;
      const dialogOptions: Electron.MessageBoxOptions = {
        type: 'info',
        buttons: [
